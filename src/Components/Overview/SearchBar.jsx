import React from 'react';

// import styled from 'styled-components';


// const ResetButton = styled.div`
//     opacity: 0;
//     transition: 0.3s;
// `

function SearchBar({ searchInput, searchItems, handleSearchInput}) {

    const handleSearch = e => {
        handleSearchInput(e.target.value)
    };

    const resetSearch = () => {
        handleSearchInput("")


    }
    return (
        <div className="px-2 search-bar">
            <img className="d-inline-blcok mx-3 search-icon" src="data:image/svg+xml;utf8;base64," />
            <input className="bg-0 w-75" type="text" placeholder="search" value={searchInput} onChange={handleSearch} />
            
            {/* <ResetButton onClick={resetSearch} style={searchInput.length>=1 && {opacity:1}}>
                x1
            </ResetButton> */}
            <button onClick={resetSearch}>x</button>
        </div>
    );
}

export default SearchBar;